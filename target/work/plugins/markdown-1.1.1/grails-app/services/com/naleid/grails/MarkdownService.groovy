...
class MarkdownService {
	 ...
	 */
	String htmlToMarkdown(def text, def customBaseUri = "", def conf = null) {
		// lazily created, so we call the method directly
	 ...
	 */
	PegDownProcessor getProcessor(def conf = null) {
		def result
		if(conf != null) {
	 ...
	 */
	Remark getRemark(def conf = null) {
		def result
		if(conf != null) {
			Map opts = getConfigurations(conf)
	...
	private Map getConfigurations(def conf) {
